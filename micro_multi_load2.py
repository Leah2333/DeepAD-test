...
path='D:/temp/k/RL_AV/micro_RL/output/1'
percent=1
HDV=1
d=1
...
micro_states = None
all_name=copy.deepcopy(veh_name)
...
while now < 4500:
    ...
    print(veh)
    for vehicle_id in all_name:
        vehicle_id2=vehicle_id[0]
        if vehicle_id2 not in veh:
            now_speed.append(0)
            now_acc.append(0)
            ...
            #print(1)
            now_lane.append(traci.vehicle.getRoadID(vehicle_id2))
            now_dist.append(traci.vehicle.getDistance(vehicle_id2))
            now_speed.append(traci.vehicle.getSpeed(vehicle_id2))
            now_acc.append(traci.vehicle.getAcceleration(vehicle_id2))
        ...
        if sum(micro_states)==0 or HDV:   
            try: micro_env.vehicle[i].current_edge
                ...
                if traci.vehicle.getSpeed(micro_env.vehicle[i].vehicle_id)<-100:
                    ...
                    now_speed2.append(traci.vehicle.getSpeed(micro_env.vehicle[i].vehicle_id))
                    now_acc2.append(traci.vehicle.getAcceleration(micro_env.vehicle[i].vehicle_id))
                    now_dist2.append(traci.vehicle.getDistance(micro_env.vehicle[i].vehicle_id))
                now_lane2.append(micro_env.vehicle[i].current_edge)
#            if i<10:
...
#                print("out of edge")
    ...
    speed_list2.append(now_speed2)
    accelation_list2.append(now_acc2)
    lane_list2.append(now_lane2)
    dist_list2.append(now_dist2)
...
if d:
    a=pd.DataFrame(speed_list2)
    a.to_csv(path+"/speed_list_d.csv",index=False)
    b=pd.DataFrame(accelation_list2)
    b.to_csv(path+"/accelation_list_d.csv",index=False)
    v=pd.DataFrame(lane_list2)
    v.to_csv(path+"/lane_list_d.csv",index=False)
    m=pd.DataFrame(dist_list2)
    m.to_csv(path+"/dist_list_d.csv",index=False)
